class DFA:
    ...
    def two_member_and_no_member(self):
        accepted=[]
        not_accepted=[]
    ...
    def isInfinite(self):
        ...
        lenght = len(self.states)
        all_str_to_2lenght = self.generator(2 * lenght)
        counter = 0
        for str in all_str_to_2lenght:
            if len(str) >= lenght and self.accept(str):
                counter += 1
                break
        if counter != 0:
            return True
    ...
    def members_of_language(self):
        ...
        if self.isInfinite():
            ...
            return members
    def members_of_language_of_len_k(self,k):
        # روش کلی اینست که تمام رشته های تا طول ان را گرفته و هر کدام که در زبان صدق میکرد را در آرایه اعضا اضافه کنیم
            all_strings_to_n = self.generator(len(self.states))
            members = []
            for string in all_strings_to_n:
                if self.accept(string) & len(string) == k:
                    members.append(string)
            return members
    
    def number_of_members_of_len_m(self,m):
            return len(self.members_of_language_of_len_k(m))
    ...
    def  small_element(self):
        ...
        if self.isInfinite():
            all_str_to_2lenght = self.generator(2 * lenght)
            for _str in all_str_to_2lenght:
                if self.accept(_str):
                    return _str
    ...
    def tarkib(self, L2):
        ...
        # استیت های پذیرش
        community_final_states = []#استیت های پذیرش اجتماع
        ...
        for states in combined_states:
            ...
            if ((current_state_1 in self.final_states)
                ...
                community_final_states.append(states)
        ...
        #اجتماع
        #print(community_final_states)
        community = [
            combined_states, L2.alphabet, initial, community_final_states,
            transition_function
        ]
        print('This is the DFA for community of Languages \n %s' % community)
